...
foreach($result as $res){
    ...
    $url_sitio = 'https://aulaunivirtual.utp.edu.co/';
    if ($res->contextid != ''){
        ...
        $url_foto = "https://univirtual.utp.edu.co/portal/archivos_nota/64ff25fa45794.f1.png"
    }
    $url_perfil = $url_sitio."user/view.php?id=".$res->userid."&course=".$id_curso;
    $url_mensaje = $url_sitio."message/index.php?id=".$res->userid;
    
    if($idioma == "en"){
        $etiqueta_enviar_mensaje = "Send message";
    }else{
        $etiqueta_enviar_mensaje = "Enviar mensaje";
    }
