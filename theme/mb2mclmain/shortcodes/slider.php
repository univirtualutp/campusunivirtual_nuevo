...
function mb2_shortcode_slider($atts, $content= null){
	...
	$sData = theme_mb2mclmain_shortcodes_slider_data($carousel_opt);
...
function mb2_shortcode_slider_item($atts, $content = null){
	...
	if (isset($GLOBALS['custom_class']) && preg_match('@mb2mclmain2@', $GLOBALS['custom_class']))
	{
	...
	if ($content || $desc || $title)
	{
		...
		if ($desc || $content)
		{
			...
			if ($link && $link_type == 1)
			{
				$readmoretext = $GLOBALS['readmoretext'] ? $GLOBALS['readmoretext'] : get_string('readmore','theme_mb2mclmain');
