...
function mb2_shortcode_courses($atts, $content= null){
	...
	$courses = theme_mb2mclmain_courses_get_items($items_opt);
	...
	if ($itemCount>1)
	{
		$output .= theme_mb2mclmain_shortcodes_content_template($courses, $items_opt);
	}
	else
	{
		$output .= get_string('nothingtodisplay');

		if (in_array(theme_mb2mclmain_site_access(),array('admin','manager','coursecreator')))
		{
			$output .= '<div>';
			$output .= '<a href="' . new moodle_url($CFG->wwwroot . '/course/edit.php',array('category'=>theme_mb2mclmain_get_category()->id)) . '">' .
