...
function mb2_shortcode_announcements($atts, $content= null){
	...
	$announcements = theme_mb2mclmain_shortcodes_announcements_get_items($items_opt);
	...
	if ($carousel)
	{
		...
		$col_cls .= ' item';
		$slider_data = theme_mb2mclmain_shortcodes_slider_data($carousel_opt);
	}
	...
	if ($itemCount>0)
	{
		$output .= theme_mb2mclmain_shortcodes_content_template($announcements, $items_opt);
	}
	else
	{
 ...
 *
 */
function theme_mb2mclmain_shortcodes_announcements_get_items ($options)
{
	...
	if (count($discussions) > 0)
	{
		foreach ($discussions as $discussion)
		{
			...
			$imgUrlAtt = theme_mb2mclmain_shortcodes_content_get_image(array('context'=>$context->id,'mod'=>'mod_forum','area'=>'attachment','itemid'=>$discussion->id));
			$imgNameAtt = theme_mb2mclmain_shortcodes_content_get_image(array('context'=>$context->id,'mod'=>'mod_forum','area'=>'attachment','itemid'=>$discussion->id), true);

			$imgUrlPost = theme_mb2mclmain_shortcodes_content_get_image(array('context'=>$context->id,'mod'=>'mod_forum','area'=>'post','itemid'=>$discussion->id));
			$imgNamePost = theme_mb2mclmain_shortcodes_content_get_image(array('context'=>$context->id,'mod'=>'mod_forum','area'=>'post','itemid'=>$discussion->id), true);
			...
			$discussion->redmoretext = get_string('readmore', 'theme_mb2mclmain');
