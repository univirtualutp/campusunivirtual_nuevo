 ...
 *
 */
function theme_mb2mclmain_user_bookmarks()
{
	global $CFG, $PAGE;
	$output = '';
	$bookmark_addremove = '';
	...
	$output .= '<a href="#" title="' . get_string('mybookmarks', 'theme_mb2mclmain') . '">';
	$output .= '<span class="text">' . get_string('mybookmarks', 'theme_mb2mclmain') . '</span>';
	...
	$output .= '<a href="#" class="theme-bookmarks-form' . $bookmark_addremove . '" data-url="' . trim($bookmarkurl) . '" data-mb2bktitle="' . theme_mb2mclmain_page_title() . '"' . $bookmark_manage_attr . '>';
	$output .= $bookmark_str;
	$output .= '</a>';
	$output .= '</li>';
 ...
 *
 */
function theme_mb2mclmain_user_bookmarks_modal ()
{
    global $PAGE, $CFG, $OUTPUT;
    $output = '';

	$createurl = new moodle_url('/theme/mb2mclmain/lib/lib_ajax_bookmarks_manage.php', array());
	$bookmarkurl = htmlspecialchars_decode(str_replace($CFG->wwwroot,'',$PAGE->url));
	$bklimit = theme_mb2mclmain_theme_setting($PAGE, 'bookmarkslimit', 15);
	...
	$output .= '<h4 class="modal-title">' . get_string('mybookmarks', 'theme_mb2mclmain') . '</h4>';
	$output .= '</div>';
	...
	get_string('bookmarkthispage','admin') . '" data-unbookmarkthispage="' . get_string('unbookmarkthispage','admin') . '" data-pagetitle="' . theme_mb2mclmain_page_title() . '" data-wentwrong="'. get_string('wentwrong','theme_mb2mclmain') . '">';
