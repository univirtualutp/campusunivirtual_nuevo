 ...
 *
 */
function theme_mb2mcl_section_module_list( $sectionid, $link = false, $active = false, $visible = false, $uservisible = true )
{
	global $PAGE;
	$output = '';
	$modules = theme_mb2mcl_get_section_activities( $sectionid, true, $visible, $uservisible );

	if ( ! count( theme_mb2mcl_get_section_activities( $sectionid, false, $visible, $uservisible ) ) )
	{
		return;
	}
...
function theme_mb2mcl_module_sections( $block = false )
{
	...
	foreach ( $sections as $section )
	{
		...
		$output .= theme_mb2mcl_section_module_list( $section['id'], true, true );
